...
$(function () {
  ...
  if (dt_basic_table.length) {
    ...
    $('div.head-label').html('<h6 class="mb-0">Liste des Produit Expert Gaming</h6>');
  ...
  $(document).on('submit', '.add', function(){
      ...
      var request   = $.ajax({
        ...
        window.location.replace("liste_rubrique_produit.php");      
        }
      });	  
  ...
  $(document).on('submit', '.edit', function(){
		  ...
		  var request   = $.ajax({
			url:          'table/php/data_liste_produit.php?job=edit_produit&id=' + id,
			cache:        true,
			data:         form_data,
			dataType:     'json',
			contentType:  'application/json; charset=utf-8',
      ...
      error : function(resultat, statut, erreur){
        Swal.fire({
          title: 'Annulée',
          text: "Une erreur s'est produite lorsxxx de l'enregistrement",
          type: 'error',
          confirmButtonClass: 'btn btn-success',
        })
      }
