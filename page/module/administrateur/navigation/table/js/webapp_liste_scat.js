...
$(function () {
  ...
  $(document).on('click', '#delete-record', function (e) {
    Swal.fire({
		  ...
		  if (result.value) {
							...
							var request = $.ajax({
							url:          'table/php/data_liste_scat.php?job=del_scat&id=' + id,
							cache:        false,
							dataType:     'json',
							...
							request.done(function(output){
								if (output.result == 'success'){
									  Swal.fire({
										  type: "success",
										  title: 'Supprimée!',
										  text: "Sous-Catégorie '" + name + "' effacé avec succès.",
  ...
  $(document).on('submit', '#jquery-val-form.add', function(){
      ...
      request.done(function(output){
        if (output.result == 'success'){         
          ...
          window.location.replace("liste_rubrique_scat.php");
  ...
  $(document).on('submit', '#jquery-val-form.edit', function(e){
		  ...
		  var request   = $.ajax({
			url:          'table/php/data_liste_scat.php?job=edit_scat&id=' + id,
			cache:        true,
			data:         form_data,
			dataType:     'json',
			contentType:  'application/json; charset=utf-8',
